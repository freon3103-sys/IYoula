console.log("✅ content.js 3.0 запущен")
...
// Пуш о новых версиях ✅
// Исправить дублирование апи запросов
...
function observeTableBody(selector, attempt = 0) {
  const MAX_ATTEMPTS = 30
  const tbody = document.querySelector(selector); // поиск нужного селектора

  if (!tbody) {
    if (attempt >= MAX_ATTEMPTS) {
      console.log(`⛔ селектор ${selector} не появился после ${MAX_ATTEMPTS} попыток — прекращаем ожидание`);
      return;
    }
    console.log(`⏳ селектора пока нет, повторяем… попытка ${attempt + 1}/${MAX_ATTEMPTS}`);

    setTimeout(() => observeTableBody(selector, attempt + 1), 500);
    return;
  }
...
async function draw_filters(attempt = 0) {
  ...
  filters.forEach((filter, index) => {
    ...
    saveItem.onclick = async () => {
      ...
      if (!apiUrl) {
      alert("Не удалось получить информацию фильтрах. Нажмите на кнопку обновления вкладки и нажмите кнопку сохранения фильтра снова.");
      return;
    }
